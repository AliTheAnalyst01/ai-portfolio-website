 ...
 * Beautiful repository cards with image carousels and project descriptions
 */

import { useState, useEffect } from 'react';
import { generateRepositoryImages } from '../lib/image-generator';

const RepositoryCard = ({ repository, analysis = null }) => {
  ...
  const loadRepositoryImages = async () => {
    try {
      setLoading(true);
      
      // Generate images using the advanced image generator
      ...
      setImages([{
        url: `https://via.placeholder.com/400x250/4F46E5/FFFFFF?text=${encodeURIComponent(repository.name)}`,
        alt: repository.name,
        description: repository.description || 'Project showcase'
      }]);
    } finally {
      setLoading(false);
    }
          ...
          <div className="border-t pt-4">
            ...
            <div className="text-center">
              <span className={`text-sm font-semibold ${getScoreColor(analysis.overall_score)}`}>
                Overall: {getScoreLabel(analysis.overall_score)}
              </span>
            </div>
          </div>
        )}
