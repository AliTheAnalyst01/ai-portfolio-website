...
export default function AnalyticsDashboard({ projects, visitorType }) {
  ...
  const portfolioStats = useMemo(() => {
    ...
    const totalContributors = projects.reduce((sum, p) => sum + (p.contributors?.length || 0), 0);
    const totalLinesOfCode = projects.reduce((sum, p) => sum + (p.size || 0), 0);
    
    // Language distribution
    const languageStats = projects.reduce((acc, project) => {
      if (project.language) {
        if (acc[project.language]) {
          acc[project.language] += 1;
        } else {
          acc[project.language] = 1;
        }
      }
    ...
    const languageData = Object.entries(languageStats)
      .map(([name, count]) => ({ name, count, percentage: (count / projects.length) * 100 }))
    ...
    const complexityData = projects.reduce((acc, project) => {
      const complexity = project.analysis?.technicalComplexity || 'intermediate';
    ...
    const priorityData = projects.reduce((acc, project) => {
      const priority = Math.floor((project.priority || 5) / 2) * 2; // Group into ranges
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="text-center">
            <div className="text-3xl font-bold text-blue-400 mb-2">
              {visitorType ? visitorType.toUpperCase() : 'GENERAL'}
            </div>
            <div className="text-sm text-gray-300">Visitor Type</div>
          </div>
          <div className="text-center">
            <div className="text-3xl font-bold text-green-400 mb-2">
              {visitorType === 'technical' ? 'Advanced' : visitorType === 'business' ? 'Intermediate' : 'General'}
            </div>
            <div className="text-sm text-gray-300">Technical Level</div>
          </div>
          <div className="text-center">
            <div className="text-3xl font-bold text-purple-400 mb-2">
              {visitorType === 'technical' ? 5 : visitorType === 'business' ? 3 : 2}
            </div>
            <div className="text-sm text-gray-300">Focus Areas</div>
          </div>
        </div>
      </motion.div>
    </div>
